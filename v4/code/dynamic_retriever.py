...
def _load_rule_cache():
    """Load cached rule→DynamicContext mappings from rule_contexts.json."""
    import os
    cache_path = os.path.join(os.path.dirname(__file__), 'rule_contexts.json')
    try:
...
_RULE_CACHE = _load_rule_cache()


def context_from_rule(content, rule_id):
    """Convert a processing rule into a retrieval context.

    First checks rule_contexts.json cache (generated by Kai analyzing
    each rule's intent). Falls back to keyword matching if not cached.
    """
    # Cache-first: look up by rule_id
    cached = _RULE_CACHE.get(str(rule_id))
    if cached:
        return DynamicContext(
            name=cached['name'],
            source=cached['source'],
            signal_words=cached.get('signal_words', []),
            w_recency=cached.get('w_recency', 1.0),
            w_keyword=cached.get('w_keyword', 1.0),
            w_importance=cached.get('w_importance', 1.0),
            w_emotion=cached.get('w_emotion', 1.0),
            w_people=cached.get('w_people', 1.0),
            w_novelty=cached.get('w_novelty', 1.0),
            w_structural=cached.get('w_structural', 1.0),
        )

    # Fallback: keyword-based matching (legacy)
